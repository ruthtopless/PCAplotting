...
server <- function(input, output, session) {
  ...
  dat2 <- brushedPoints(PCAtestdata[,c(input$xcol2,input$ycol2,"PCAETHBROAD","PCAETHSPECIFIC")], input$plot1_click, xvar=input$xcol, yvar=input$ycol)
  ...
  #output$plot1 <- renderPlot({
  #  ggplot(dat, aes(x = dat[,1], y = dat[,2], colour = dat$colPoints)) + geom_point() + ggtitle("title") + xlab(input$xcol) + ylab(input$ycol)
  #})
...
output$plot2 <- renderPlot({
  palette <-c("#999999", "#E41A1C", "#377EB8", "#4DAF4A", "#984EA3","#FF7F00", "#FFFF33", "#A65628", "#F781BF", "#015249", "#76323F", "#000080", "#49274A")
  #palette<-c("#E24E42", "#E9B000", "#EB6E80", "#008F95", "#94618E", "#49274A", "#4484CE", "#015249", "#F7882F", "#8FC33A", "#535353", "#B37D4E", "#76323F")
  dat2 <- brushedPoints(PCAtestdata[,c(input$xcol2,input$ycol2,"PCAETHBROAD","PCAETHSPECIFIC")], input$plot1_click, xvar=input$xcol2, yvar=input$ycol2)
  dat2$colPoints <- palette[as.factor(dat2$PCAETHSPECIFIC)]
  x.max <- round.up(max(dat2[,1], na.rm = TRUE), decimals = 3)
  x.min <- round.down(min(dat2[,1], na.rm = TRUE), decimals = 3)
  y.max <- round.up(max(dat2[,2], na.rm = TRUE), decimals = 3)
  y.min <- round.down(min(dat2[,2], na.rm = TRUE), decimals = 3)
  plot(x=(dat2[,1]),y=(dat2[,2]),
       col = dat2$colPoints,
       xlim = c(x.min, x.max), ylim = c(y.min, y.max),
       xlab = input$xcol2, ylab = input$ycol2,
       pch = 20, cex = 2)
  dataEllipse(x=(dat2[,1]), y=(dat2[,2]), groups = factor(dat2$PCAETHSPECIFIC), center.cex = 0, robust = TRUE, grid = FALSE, levels = 0.8, col = palette, add = TRUE, plot.points = FALSE, group.labels = NA)
  legend(x = "topright", legend = levels(factor(dat2$PCAETHSPECIFIC)), col = palette, lwd = 6, bty = "n", cex = 1)
  #points(dat, pch = 3, cex = 1, lwd = 4)
})
...
output$click_info2 <- renderPrint({
  nearPoints(PCAtestdata[,c("SUBJECT",input$xcol2,input$ycol2,"PCAETHBROAD","PCAETHSPECIFIC")], input$plot2_click, xvar=input$xcol2, yvar=input$ycol2, addDist = TRUE)
})
