...
server <- function(input, output, session) {
  ...
  output$downloadDataPlot1 <- downloadHandler(
    filename = function() {paste('plot1-', Sys.Date(), Sys.time(), '.csv', sep='')},
    content = function(file) {write.csv(selectedData(), file)})
  output$downloadImagePlot1 <- downloadHandler(
    filename = function() { paste('plot1-', Sys.Date(), Sys.time(), '.png', sep='') },
    content = function(file) {ggsave(file,plotInput())}) 
  ...
  output$brush_info <- renderDataTable({ plot1brushselected()},
                                       ...
                                       options = list(scrollY = "300px", scrollCollapse = TRUE, paging = FALSE) #for vertical scrolling
  )
  ...
  output$downloadDataPlot2 <- downloadHandler(
    filename = function() {paste('plot2-', Sys.Date(), Sys.time(), '.csv', sep='')},
    content = function(file) {write.csv(plot1brushselected2(), file)})
  
  output$downloadImagePlot2 <- downloadHandler(
    filename = function() { paste('plot2-', Sys.Date(), Sys.time(), '.png', sep='') },
    content = function(file) {ggsave(file,plotInput2())})
